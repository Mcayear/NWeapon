...
import { version, _C, TaskExecList, CommandList,
getItData, getArmorConfig, getGemConfig, getJewelryConfig, getPaperConfig, getRuneConfig, getWeaponConfig } from './util/WeaponConfig.js';
import { mc, File, ParamType, PermType } from '@LLSELib';
...
import { ForgingFakeInvChange } from './improvements/forging/ForgingFakeInvChange.js';
import { NWeaponDecomposition } from './improvements/Decomposition/NWeaponDecomposition.js' 
import { checkAttr } from './improvements/check/check.js';
...
function NWeaponCommandHandle(_cmd, _ori, out, res) {
    ...
    switch (res.action) {
        ...
        case "addnbt": {
            ...
            File.writeTo("./plugins/BlocklyNukkit/UseItemExec/NbtItem.yml", Util.JSONtoYAML(JSON.stringify(_C.NbtItem)));
            break;
        }
        case "delnbt": {
        ...
        case "reload": {
            ...
            _C.MainConfig = JSON.parse(Util.YAMLtoJSON(manager.readFile("./plugins/BlocklyNukkit/NWeapon/Config.yml")));
        ...
        case "drop": {
            ...
            let item = onlyNameGetItem(res.ItemType, res.ItemName, res.number, player);
            if (!item) return;
            if (res.action === "give") {
                if (res.playerName) {
...
export function main() {
    ...
    //['nwe effect <玩家名> <属性名> [值] [时长]', '给予指定玩家指定时长的属性值']
